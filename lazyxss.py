...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='XSS Vulnerability Checker v1.0 by IAMUNIXTZ',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Examples:
  python3 lazyxss.py              # Interactive mode
  python3 lazyxss.py -h           # Show this help message
        '''
    )
    parser.add_argument('-v', '--version', action='version', version='LazyXss v1.0')
    
    args = parser.parse_args()
    
    # If we reach here, we're not showing help, so continue with normal execution
    print(f"""
{GREEN}
 /$$                                     /$$   /$$                   
| $$                                    | $$  / $$                   
